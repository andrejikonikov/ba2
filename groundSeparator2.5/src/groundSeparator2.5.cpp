...
int main (int argc, char** argv) {
    if (argc < 2) {
        ...
        for (int i = 0; i < pointCloudSize; i++) {
            if(mn > cloud->points[i].x) {
                mn = cloud->points[i].x;
            } else {
                if(mx < cloud->points[i].x) {
                    mx = cloud->points[i].x;
                }
            }
        }

        float cloudLength = mx - mn;
        float step = cloudLength / num_threads;

        std::cout << "min=" << mn << " max="<< mx << " len="<< cloudLength <<'\n';
        ...
        for (int i = 0; i < num_threads; i++) {
            ...
            for (int j = 0; j < pointCloudSize; j++) {
                if ((cloud->points[j].x >= mn + (step * i)) && (cloud->points[j].x < mn + (step * i) + step )) {
                    // create point
            ...
            std::cerr << temp_point_cloud.points.size () << std::endl;
